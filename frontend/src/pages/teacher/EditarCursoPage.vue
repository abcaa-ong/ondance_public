<template>
  <q-page class="q-pa-lg">
    ...
    <!-- Loading state -->
    <div v-if="loading" class="column items-center justify-center" style="min-height: 300px; gap: 12px;">
      <q-spinner size="32px" :style="{ color: 'var(--od-accent)' }" />
      <span style="font-size: 13px; color: var(--od-text-4);">Carregando curso...</span>
    </div>

    <!-- Error state -->
    <div v-else-if="loadError" class="column items-center justify-center" style="min-height: 300px; gap: 12px;">
      <q-icon name="error_outline" size="40px" style="color: var(--od-text-5);" />
      <p style="font-size: 14px; color: var(--od-text-3); margin: 0;">{{ loadError }}</p>
      <q-btn flat no-caps label="Voltar" style="color: var(--od-accent);" @click="$router.push('/teacher/courses')" />
    </div>
    ...
    <template v-else>
      ...
      <div class="row items-center q-mb-lg" style="gap: 12px;">
        ...
        <div>
          <div class="od-display" style="font-size: 22px; color: var(--od-text-1);">Editar Curso</div>
          <p style="color: var(--od-text-3); margin: 2px 0 0; font-size: 13px;">Edite as informações e a estrutura do curso</p>
        </div>
      </div>
...
onMounted(async () => {
  try {
    ...
    if (err.response?.status === 404) {
      ...
      loadError.value = 'Erro ao carregar o curso. Tente novamente.'
    }
  } finally {
    loading.value = false
  }
